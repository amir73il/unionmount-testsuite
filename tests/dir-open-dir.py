...
# Open read-only
def subtest_1(ctx):
    ctx.begin_test(1, "Open O_DIRECTORY | O_RDONLY")
...
def subtest_2(ctx):
    ctx.begin_test(2, "Open O_DIRECTORY | O_WRONLY")
    f = ctx.non_empty_dir() + ctx.termslash()
...
def subtest_3(ctx):
    ctx.begin_test(3, "Open O_DIRECTORY | O_WRONLY * 2")
    f = ctx.non_empty_dir() + ctx.termslash()
...
def subtest_4(ctx):
    ctx.begin_test(4, "Open O_DIRECTORY | O_APPEND|O_WRONLY")
    f = ctx.non_empty_dir() + ctx.termslash()
...
def subtest_5(ctx):
    ctx.begin_test(5, "Open O_DIRECTORY | O_RDWR")
    f = ctx.non_empty_dir() + ctx.termslash()
...
def subtest_6(ctx):
    ctx.begin_test(6, "Open O_DIRECTORY | O_APPEND|O_RDWR")
    f = ctx.non_empty_dir() + ctx.termslash()

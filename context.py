...
###############################################################################
class test_context:
    def __init__(self, cfg, termslash=False, direct_mode=False, recycle=False, remount=None, max_layers=0):
        self.__cfg = cfg
    ...
    def note_lower_layer(self, path):
        self.__lower_layer = path
    ...
    def check_copy_up(self, filename, dentry, layer, blocks):
        if self.config().testing_snapshot():
            ...
            # Check that parent is copied to snapshot
